...
root = os.getenv('ROOT')
data_path = os.path.join(root, 'data_understand_and_preprocessing', 'data')
class analyzer:
    def __init__(self):
        self.metadata_path = data_path + '\\Metadata.csv'
        self.videos_path = data_path + '\\Videos.csv'
        self.labels_path = data_path + '\\Labels.csv'
        self.analystic_result_path = data_path + '\\Analysis_result.csv'
    def readcsv(self,file): #Get a general understanding of your dataset structure and cleanliness.
        df = pd.read_csv(file)
        return df
    def yc1(self):
        metadata = self.readcsv(self.metadata_path)
        videos = self.readcsv(self.videos_path)
        labels = self.readcsv(self.labels_path)
        analysis_result = self.readcsv(self.analystic_result_path)
        metadata.info()
        videos.info()
        labels.info()
    ...
    def yc4(self): #Explore which features correlate with violence probability.
        df = self.readcsv(self.analystic_result_path)
