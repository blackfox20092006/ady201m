...
def process_single_video(task_info):
    ...
    try:
        ...
        if proc.returncode != 0:
            error_message = err_bytes.decode('utf-8', errors='ignore').strip()
            return f"FAILED (ffmpeg error): '{filename}'. Details: {error_message}"
        ...
        return f"SUCCESS: '{filename}' -> '{os.path.basename(output_path)}' | Shape: {tensor.shape}"
