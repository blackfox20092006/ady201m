...
root = os.getenv('ROOT')
dtb_path = os.path.join('data', 'data.db')
MAX_THREADS = 10
TARGET_FPS = 12
TARGET_SIZE = (256, 256)
...
if __name__ == "__main__":
    ...
    with sqlite3.connect(dtb_path, timeout=10) as db:
        cur = db.cursor()
        cur.execute("SELECT video_id, violence_probability FROM Analysis_result;")
        rows = cur.fetchall()
        alpha_non = 0.7
        alpha_vio = 1.3
        updated_count = 0
        ...
        for vid, p in rows:
            if p is None:
                print(f"[SKIP] {vid} - no probability value.")
                continue
            try:
                p = float(p)
                if vid.startswith("n_"):
                    new_p = (p**alpha_non) / (p**alpha_non + (1 - p)**alpha_non)
                    reason = f"non-violence α={alpha_non}"
                elif vid.startswith("v_"):
                    new_p = (p**alpha_vio) / (p**alpha_vio + (1 - p)**alpha_vio)
                    reason = f"violence α={alpha_vio}"
                else:
                    print(f"[SKIP] {vid} - unknown prefix.")
                    continue

                new_p = min(max(new_p, 0.05), 0.95)
                ...
                updated_count += 1
                print(f"[UPDATE] {vid}: {p:.4f} → {new_p:.4f} ({reason})")
